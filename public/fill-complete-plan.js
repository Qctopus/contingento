/**
 * COMPLETE PLAN FILLER
 * 
 * This script does EVERYTHING needed to review a complete business continuity plan:
 * 1. Sets industry and location
 * 2. Fills business characteristics  
 * 3. Fills ALL user-fillable wizard fields
 * 4. Provides data for system to generate risks and strategies
 * 
 * Just paste this in browser console and press Enter
 */

(function fillCompletePlan() {
  console.log('🎯 Filling COMPLETE business continuity plan...')
  
  // =============================================================================
  // STEP 1: Set Industry and Location (triggers API for risks/strategies)
  // =============================================================================
  ...
  const preFillData = {
    ...
    // These will be populated by the system based on industry/location
    hazards: [],
    preFilledFields: {},
    contextualExamples: {},
  ...
  // =============================================================================
  // STEP 2: Fill ALL User-Entered Data
  // =============================================================================
  ...
  const formData = {
    ...
    ESSENTIAL_FUNCTIONS: {
      'Guest Services': ['front_desk', 'room_service', 'concierge', 'housekeeping'],
      'Food & Beverage': ['restaurant_operations', 'bar_service', 'kitchen_food_prep', 'inventory_ordering'],
      'Facilities & Maintenance': ['building_maintenance', 'pool_beach_maintenance', 'hvac', 'water_supply', 'electrical_power'],
      'IT & Communications': ['reservation_system', 'payment_processing', 'wifi_internet', 'phone_systems', 'website_booking'],
      'Finance & Administration': ['payroll', 'accounts', 'financial_reporting', 'compliance', 'insurance'],
      'Security & Safety': ['guest_security', 'property_security', 'emergency_response', 'safety_procedures']
    },
    ...
    RISK_ASSESSMENT: {
      'Risk Assessment Matrix': [
        // ADMIN2 DATABASE - ONLY THESE 6 RISK TYPES EXIST in AdminUnitRisk table:
        // hurricane, flood, earthquake, drought, landslide, power_outage
        { 'Hazard': 'Hurricane', 'Likelihood': 'Likely (7-8)', 'Severity': 'Catastrophic (9-10)', 'Risk Level': 'Very High', 'Recommended Actions': 'Install hurricane shutters on all windows and glass doors; Maintain 72-hour emergency supply kit including water, food, first aid, flashlights, batteries; Establish guest and staff evacuation procedures and assembly points; Ensure backup power generation capacity for critical systems; Secure outdoor furniture and equipment; Establish agreements with mainland hotels for guest relocation; Train staff on hurricane preparedness annually before season', isSelected: true, hazardId: 'hurricane' },
        { 'Hazard': 'Flood', 'Likelihood': 'Possible (4-6)', 'Severity': 'Major (7-8)', 'Risk Level': 'High', 'Recommended Actions': 'Install drainage systems and flood barriers; Store sandbags and emergency supplies; Elevate critical equipment; Maintain emergency contact list for flood response', isSelected: true, hazardId: 'flood' },
        { 'Hazard': 'Earthquake', 'Likelihood': 'Unlikely (2-3)', 'Severity': 'Major (7-8)', 'Risk Level': 'Medium', 'Recommended Actions': 'Secure heavy equipment and furniture; Install flexible utility connections; Train staff on earthquake safety; Maintain emergency supplies', isSelected: true, hazardId: 'earthquake' },
        { 'Hazard': 'Drought', 'Likelihood': 'Possible (4-6)', 'Severity': 'Moderate (4-6)', 'Risk Level': 'Medium', 'Recommended Actions': 'Install water storage tanks; Implement water conservation measures; Establish relationships with emergency water suppliers', isSelected: true, hazardId: 'drought' },
        { 'Hazard': 'Landslide', 'Likelihood': 'Unlikely (1-2)', 'Severity': 'Minor (2-3)', 'Risk Level': 'Low', 'Recommended Actions': 'Monitor slope stability; Install drainage systems; Regular inspection of surrounding terrain', isSelected: true, hazardId: 'landslide' },
        { 'Hazard': 'Power Outage', 'Likelihood': 'Possible (4-6)', 'Severity': 'Major (7-8)', 'Risk Level': 'High', 'Recommended Actions': 'Maintain three backup generators with capacity for critical operations (refrigeration, elevators, emergency lighting, water pumps); Keep minimum 72-hour fuel reserve on-site with agreements for emergency fuel delivery; Install battery backup systems for reservation and payment systems; Develop manual procedures for check-in, billing, and essential services; Test generators monthly and under load quarterly', isSelected: true, hazardId: 'power_outage' }
      ]
    },
    
    STRATEGIES: {
      'Prevention Strategies (Before Emergencies)': ['maintenance', 'physical_security', 'cybersecurity', 'insurance', 'employee_training', 'supplier_diversity', 'financial_reserves', 'data_backup', 'building_upgrades', 'emergency_supplies', 'risk_monitoring'],
      'Response Strategies (During Emergencies)': ['emergency_team', 'safety_procedures', 'emergency_communication', 'alternative_locations', 'emergency_inventory', 'customer_continuity', 'emergency_services', 'closure_procedures', 'essential_operations'],
      'Recovery Strategies (After Emergencies)': ['damage_assessment', 'insurance_claims', 'business_resumption', 'employee_support', 'customer_retention', 'supplier_restoration', 'reputation_management', 'facility_repair', 'equipment_replacement', 'lessons_learned'],
  ...
  try {
    ...
    console.log('✅ COMPLETE business continuity plan saved!')
    console.log('')
    console.log('📊 What was filled:')
    console.log('  ✅ Industry: Hospitality & Tourism')
    console.log('  ✅ Location: Christ Church, Barbados')
    console.log('  ✅ Business Characteristics: 8 key factors')
    console.log('  ✅ Plan Information (8 fields)')
    console.log('  ✅ Business Overview (4 comprehensive sections)')
    console.log('  ✅ Essential Functions (6 categories, 24 functions)')
    console.log('  ✅ Function Priorities (9 prioritized functions)')
    console.log('  ✅ Risk Assessment (6 detailed risks)')
    console.log('  ✅ Strategies (32 selected + long-term plan)')
    console.log('  ✅ Action Plan (6-person team + 3 detailed plans)')
    console.log('  ✅ Contacts (20+ contacts across 4 categories)')
    console.log('  ✅ Vital Records (8 critical record types)')
    console.log('  ✅ Testing & Maintenance (Complete schedules & metrics)')
    console.log('')
    console.log('🔄 Refreshing page in 2 seconds...')
    console.log('📄 After refresh: Navigate to the LAST step to see the complete plan document')
    ...
    return { 
      success: true, 
      sections: Object.keys(formData).length,
      message: 'Complete plan ready for review'
    }
