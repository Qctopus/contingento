...
const CompactTable = ({ 
  ...
  if (!Array.isArray(data) || data.length === 0) return <p className="text-gray-500 italic">No data available</p>
  ...
  useEffect(() => {
    const loadStrategies = async () => {
      try {
        ...
        // Fallback logic has been removed to prevent loading of hardcoded data.
        setStrategies([])
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 print:py-2 space-y-6 print:space-y-4">
      ...
      <div className="grid lg:grid-cols-3 gap-6 print:gap-4 print:break-inside-avoid">
        <CompactCard className="print:break-inside-avoid">
          ...
          <InfoGrid items={[
            ...
            { label: 'Plan Manager', value: getLocalizedText(formData.PLAN_INFORMATION?.['Plan Manager'], locale) },
            { label: 'Alternate Manager', value: getLocalizedText(formData.PLAN_INFORMATION?.['Alternate Manager'], locale) },
          ]} />
        </CompactCard>
        ...
        <CompactCard className="lg:col-span-2 print:break-inside-avoid">
          ...
          <InfoGrid items={[
            ...
            { label: 'Digital Plan Location', value: getLocalizedText(formData.PLAN_INFORMATION?.['Digital Plan Location'], locale) },
            { label: 'Physical Plan Location', value: getLocalizedText(formData.PLAN_INFORMATION?.['Physical Plan Location'], locale) },
          ]} />
        </CompactCard>
        </div>
      ...
      <CompactCard className="print:break-inside-avoid">
          ...
          <div>
            ...
            <div className="bg-blue-50 rounded-lg p-4 text-sm text-gray-800">
              {(() => {
                ...
                return typeof purpose === 'string' ? purpose : (getLocalizedText(purpose, locale) || 'Not specified')
              })()}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-green-50 rounded-lg p-4 text-sm text-gray-800">
              {(() => {
                ...
                return typeof products === 'string' ? products : (getLocalizedText(products, locale) || 'Not specified')
              })()}
            </div>
            </div>
          </div>
        ...
        {formData.ESSENTIAL_FUNCTIONS?.['Business Functions'] && Array.isArray(formData.ESSENTIAL_FUNCTIONS['Business Functions']) && (
          <div className="mt-6">
                ...
                <tbody>
                  {formData.ESSENTIAL_FUNCTIONS['Business Functions'].map((func: any, idx: number) => {
                    const getFieldValue = (field: any) => {
                      if (!field) return '-'
                      return typeof field === 'string' ? field : (getLocalizedText(field, locale) || '-')
                    }
         ...
         formData.RISK_ASSESSMENT['Risk Assessment Matrix'].length > 0 ? (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-gray-50 rounded-lg p-6 border border-blue-200">
              ...
              {(() => {
                // Only count user-selected risks (must be explicitly true, not just absence of false)
                const risks = formData.RISK_ASSESSMENT['Risk Assessment Matrix'].filter((r: any) => r.isSelected === true)
            ...
            <div className="space-y-4">
              {formData.RISK_ASSESSMENT['Risk Assessment Matrix']
                .filter((risk: any) => risk.isSelected === true) // Only show explicitly selected risks
                  ...
                  <div key={index} className={`border-l-4 ${borderColor} bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow print:break-inside-avoid`}>
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        {risk.description && (() => {
                          const description = getLocalizedText(risk.description, locale)
                          return description ? (
                      ...
                      <span className={`${badgeColor} px-4 py-2 rounded-full text-sm font-bold ml-4 whitespace-nowrap shadow-sm`}>
                        {typeof risk['Risk Level'] === 'string' ? risk['Risk Level'] : getLocalizedText(risk['Risk Level'], locale) || 'High'}
                      </span>
                    </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        ...
                        <div className="text-lg font-bold text-gray-900">
                          {(() => {
                            ...
                            return typeof severity === 'string' ? severity : getLocalizedText(severity, locale) || '-'
                          })()}
                        </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        ...
                        {risk.riskLevel && (
                          <div className="text-xs text-gray-500 mt-1">
                            {typeof risk.riskLevel === 'string' ? risk.riskLevel : getLocalizedText(risk.riskLevel, locale)}
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="border-t pt-4 space-y-2">
                        {risk.riskCategory && (() => {
                          const category = typeof risk.riskCategory === 'string' 
                            ? risk.riskCategory 
                            : getLocalizedText(risk.riskCategory, locale)
                          return category ? (
                        ...
                        {risk.vulnerabilities && (() => {
                          const vulnerabilities = getLocalizedText(risk.vulnerabilities, locale)
                        ...
                        {risk.affectedFunctions && (() => {
                          const affectedFunctions = getLocalizedText(risk.affectedFunctions, locale)
                        ...
                        {risk.potentialImpact && (() => {
                          const potentialImpact = getLocalizedText(risk.potentialImpact, locale)
                                  ...
                                  <div>
                                    ...
                                    {strategy.estimatedDIYSavings && (
                                      <p className="text-xs text-green-600 mt-1 font-medium">
                                        Potential savings: {getLocalizedText(strategy.estimatedDIYSavings, locale)}
                                      </p>
                                    )}
                                  </div>
                                )}
      ...
      <CompactCard className="print:break-before-page">
        ...
        {(() => {
          // Get high/extreme priority risks that user actually selected
          ...
          const priorityRisks = riskMatrix.filter((r: any) => {
            ...
            // PRODUCTION-READY: Only show risks explicitly selected by user
            // This filters out test data that doesn't have isSelected property
            const isUserSelected = r.isSelected === true
          ...
          return (
            <div className="space-y-8">
              ...
              {priorityRisks.map((risk: any, riskIndex: number) => {
                ...
                // PRODUCTION-READY: Only show risks that have actual selected strategies
                // This ensures we never show "0 specific steps" or "No Action Plan Data Available"
                // If a risk has no strategies, it shouldn't appear in the detailed action plans section
                if (relevantStrategies.length === 0) {
                  console.log(`⚠️ Skipping ${hazardName} - no selected strategies`)
                  return null
                }
                
                console.log(`✅ Showing ${hazardName} with ${relevantStrategies.length} strategies`)
                                    ...
                                    <div className="flex flex-wrap gap-4 text-sm">
                                      {step.estimatedCostJMD && (
                                        <div className="text-gray-700">
                                          <span className="font-medium">Cost:</span> {getLocalizedText(step.estimatedCostJMD, locale)}
                                        </div>
                                      )}
                                ...
                                <div className="flex items-start mb-3">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {step.whyThisStepMatters && (
                                      <div className="bg-blue-50 rounded-lg p-3 mb-3">
                                        <div className="text-xs font-semibold text-blue-900 mb-1">Why This Matters:</div>
                                        <p className="text-sm text-blue-800">{getLocalizedText(step.whyThisStepMatters, locale)}</p>
                                      </div>
                                    )}
                                  ...
                                  <div className="flex flex-wrap gap-3 text-xs">
                                    ...
                                    {strategy.complexityLevel && (
                                      <div className="bg-gray-50 px-3 py-1 rounded">
                                        <span className="font-medium">Difficulty:</span> {getLocalizedText(strategy.complexityLevel, locale)}
                                      </div>
                                    )}
                                  </div>
                                </div>
            ...
            {formData.TESTING_AND_MAINTENANCE['Annual Review Process'] && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-green-800">
                    {getLocalizedText(formData.TESTING_AND_MAINTENANCE['Annual Review Process'], locale)}
                </div>
              </div>
            )}
            ...
            {formData.TESTING_AND_MAINTENANCE['Trigger Events for Plan Updates'] && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  ...
                  <div className="text-sm text-orange-800">
                    {getLocalizedText(formData.TESTING_AND_MAINTENANCE['Trigger Events for Plan Updates'], locale)}
                </div>
              </div>
            )}
            </div>
      ...
      <CompactCard className="bg-gradient-to-r from-gray-50 to-gray-100">
        <div className="text-center py-4">
          <div className="flex items-center justify-center space-x-6 mb-3 text-sm text-gray-700">
            <div>
              <span className="font-medium">Prepared By:</span> {getLocalizedText(formData.PLAN_INFORMATION?.['Plan Manager'], locale) || 'Plan Manager'}
            </div>
            <div>
